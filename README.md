# RCS automated 
Automated Restricted Cubic Spline (RCS) analysis and visualisation in Stata

**RCS Linear Regression_single**	-	
Testing non-linearity in an association between a continuous exposure and a continuous outcome in a linear regression model 
...
'if' must be included in the 'if clause', otherwise you get an error. The command line should be like: global iff if indicator==1. However, it can be left empty if there is no need, like: global iff.  
